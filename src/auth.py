...
class TokenService:
    ...
    @staticmethod
    def decode(token: str, expected_role: str, suppress: bool = False) -> Dict[str, Any]:
        try:
            decoded = jwt.decode(
                token,
                settings.JWT_SECRET,
            ...
            if suppress:
                decoded = jwt.decode(
                    token, settings.JWT_SECRET, algorithms=["HS256"], options={"verify_signature": False}
                )
                if decoded["role"] != expected_role:
                    raise errors.token_validation_failed()
                return decoded
...
class SessionService:
    ...
    def create_session(user: User, is_long: bool, request: Request, db: Session) -> UserSession:
        ...
        session = UserSession(
            user_id=user.id,
            fingerprint=SessionService.get_user_agent_info(request),
...
def init_user_tokens(user: User, is_long: bool, request: Request, response: Response, db: Session) -> Refresh:
    session = SessionService.create_session(user, is_long, request, db)
    access = TokenService.create_access_token(session.id, session.identity)
    refresh = TokenService.create_refresh_token(session.id, session.identity, is_long, session.invalid_after)
...
def verify_user_access(access: str, request: Request, db: Session) -> UserSession:
    access_payload = TokenService.decode(access, "access")
    session = db.query(UserSession).get(access_payload["session"])
    if not session or session.fingerprint != SessionService.get_user_agent_info(request) \
            or session.identity != access_payload["identity"]:
        db.delete(session)
        db.commit()
...
def refresh_user_tokens(access: str, refresh: str, request: Request, response: Response, db: Session) -> Refresh:
    access_payload = TokenService.decode(access, "access", suppress=True)
    refresh_payload = TokenService.decode(refresh, "refresh")
